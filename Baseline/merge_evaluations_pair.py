    ...
    try:
        ...
        for p in prompts:
            ...
            for c in p['continuations']:
                sub_max_toxicities.append(c[attr])
            try:
                tox = max(sub_max_toxicities)
                max_toxicities.append(tox)
            except:
                print("invalid generations!!!")
...
if __name__ == '__main__':
    ...
    if 'innerdetox_hook' in config:
        if 'vector_method' in config.innerdetox_hook:
            ...
            eval_fp_ppl = f'{args.fn}/{fp}({vector_method}).jsonl'
            eval_fp_toxicity = f'{args.fn}/{fp}({vector_method})({args.toxicity_type}).jsonl'
        else:
            output_fp = in_dir
            eval_fp_ppl = f'{args.fn}/{fp}.jsonl'
            eval_fp_toxicity = f'{args.fn}/{fp}({args.toxicity_type}).jsonl'
    else:
        output_fp = in_dir
        eval_fp_ppl = f'{args.fn}/{fp}.jsonl'
        eval_fp_toxicity = f'{args.fn}/{fp}({args.toxicity_type}).jsonl'
    ...
    if args.toxicity_type == "perspective_api_toxicity":
        evals_toxicity = list(jsl.open(eval_fp_toxicity, 'r'))
    ...
    elif args.toxicity_type == "llamaguard_toxicity" or \
    ...
    # 将结果逐行追加保存到 JSONL 文件
    with jsl.open(f"{args.fn}/all_results.jsonl", mode='a') as writer:
        writer.write(res_print)
