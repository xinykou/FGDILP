...
def generate_negatives(model, input_ids, num_to_generate=6):
    ...
    beam_output = model.generate(input_ids, max_length=50, do_sample=True, num_beams=num_to_generate,
                                 num_return_sequences=num_to_generate, early_stopping=True)
    # print(beam_output)
...
def get_score(model, x, y, i, j):
    outputs = model(input_ids=x['input_ids'][i].reshape(1, -1), attention_mask=x['attention_mask'][i].reshape(1, -1),
...
def compute_contrastive_loss(model, x, y, tokenizer=None, device=None):
    loss = 0
