...
import os
import random
from API_evaluations.chatgpt_score_toxicity_or_not import context_level_toxicity_classifier
...
from API_evaluations.model_server import fluency
...
def run_ppl_evaluation_with_llama(prompts, continuations, config=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fn",
                        type=str,
                        default=None)
    parser.add_argument("--eval_num", type=int, default=None)   # 评估的 ppl_davinci 采样的数量
    ...
    if 'goodtriever' in config.model_type:
        pass
    else:
    ...
    if 'goodtriever' in config.model_type:
        in_dir = config.custom_path
        output_fp = osp.join(args.fn, fp + '.jsonl')
    else:
        in_dir = f'{args.fn}/{fp}.jsonl'
    ...
    if args.eval_type == "chatgpt_api_toxicity":
        invalid_score_num = 0
        score_key = 'toxicity'
        scores, error = context_level_toxicity_classifier(prompts, continuations)
        invalid_score_num += error
        print(f'invalid score num: {invalid_score_num}')
    elif args.eval_type == "ppl_llama2-13b" or \
            args.eval_type == "ppl_llama2-13b-chat" or \
            args.eval_type == "ppl_vicuna-13b-v1.5":
        score_key = 'ppl'
        scores = run_ppl_evaluation_with_llama(prompts, continuations, config=config)
