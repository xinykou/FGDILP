...
def topk_values_mask(M, K=0.7, return_mask=False):
    ...
    if K < 1:
        ...
        if M.dtype == torch.bfloat16:
            kth_values, _ = M.float().abs().kthvalue(k, dim=2, keepdim=True)
            kth_values = kth_values.bfloat16()  # 转换回 'BFloat16'
        else:
...
def disjoint_merge(Tensor, merge_func, sign_to_mult=None):  # sign_to_mult: batch, head*dim; Tensor: batch, n_neg, head*dim
